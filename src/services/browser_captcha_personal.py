...
class BrowserCaptchaService:
    ...
    async def start_resident_mode(self, project_id: str):
        ...
        for retry in range(60):
            try:
                ...
                ready_state = await self.resident_tab.evaluate(
                    "document.readyState")
                debug_logger.log_info(
                    f"[BrowserCaptcha] 页面状态: {ready_state} (重试 {retry + 1}/60)"
                )
                ...
                debug_logger.log_warning(
                    f"[BrowserCaptcha] 标签页连接丢失: {e}，尝试重新获取...")
                # 标签页可能已关闭，尝试重新创建
                try:
                    self.resident_tab = await self.browser.get(website_url,
                                                               new_tab=True)
                ...
                debug_logger.log_warning(
                    f"[BrowserCaptcha] 等待页面异常: {e}，重试 {retry + 1}/15...")
                await asyncio.sleep(2)
        ...
        # 等待 reCAPTCHA 加载
        self._recaptcha_ready = await self._wait_for_recaptcha(
            self.resident_tab)
        ...
        self._running = True
        debug_logger.log_info(
            f"[BrowserCaptcha] ✅ 常驻模式已启动 (project: {project_id})")
    ...
    async def stop_resident_mode(self, project_id: Optional[str] = None):
        ...
        async with self._resident_lock:
            if project_id:
                ...
                debug_logger.log_info(
                    f"[BrowserCaptcha] 已关闭 project_id={project_id} 的常驻模式")
            else:
                # 关闭所有常驻标签页
                ...
                for pid in project_ids:
                    ...
                    if resident_info and resident_info.tab:
                        try:
                            ...
                            pass
                debug_logger.log_info(
                    f"[BrowserCaptcha] 已关闭所有常驻标签页 (共 {len(project_ids)} 个)")
    ...
    async def _wait_for_recaptcha(self, tab) -> bool:
        ...
        for i in range(20):
            ...
            if is_enterprise:
                debug_logger.log_info(
                    f"[BrowserCaptcha] reCAPTCHA Enterprise 已加载（等待了 {i * 0.5} 秒）"
                )
                return True
            await tab.sleep(0.5)
            ...
            (() => {{
                ...
                try {{
                    grecaptcha.enterprise.ready(function() {{
                        grecaptcha.enterprise.execute('{self.website_key}', {{action: 'VIDEO_GENERATION'}})
                            .then(function(token) {{
        ...
        for i in range(30):
            ...
            if error:
                debug_logger.log_error(
                    f"[BrowserCaptcha] reCAPTCHA 错误: {error}")
                break
    ...
    async def get_token(self, project_id: str) -> Optional[str]:
        ...
        async with self._resident_lock:
            ...
            if resident_info is None:
                debug_logger.log_info(
                    f"[BrowserCaptcha] project_id={project_id} 没有常驻标签页，正在创建..."
                )
                resident_info = await self._create_resident_tab(project_id)
                if resident_info is None:
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] 无法为 project_id={project_id} 创建常驻标签页，fallback 到传统模式"
                    )
                    return await self._get_token_legacy(project_id)
                self._resident_tabs[project_id] = resident_info
                debug_logger.log_info(
                    f"[BrowserCaptcha] ✅ 已为 project_id={project_id} 创建常驻标签页 (当前共 {len(self._resident_tabs)} 个)"
                )
        ...
        if resident_info and resident_info.recaptcha_ready and resident_info.tab:
            ...
            debug_logger.log_info(
                f"[BrowserCaptcha] 从常驻标签页即时生成 token (project: {project_id})..."
            )
            try:
                ...
                if token:
                    debug_logger.log_info(
                        f"[BrowserCaptcha] ✅ Token生成成功（耗时 {duration_ms:.0f}ms）"
                    )
                    return token
                else:
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] 常驻标签页生成失败 (project: {project_id})，尝试重建..."
                    )
            except Exception as e:
                debug_logger.log_warning(
                    f"[BrowserCaptcha] 常驻标签页异常: {e}，尝试重建...")
            ...
            async with self._resident_lock:
                ...
                if resident_info:
                    ...
                    # 重建后立即尝试生成
                    try:
                        token = await self._execute_recaptcha_on_tab(
                            resident_info.tab)
                        if token:
                            debug_logger.log_info(
                                f"[BrowserCaptcha] ✅ 重建后 Token生成成功")
                            return token
                    except Exception:
                        pass
    ...
    async def _create_resident_tab(
            self, project_id: str) -> Optional[ResidentTabInfo]:
        ...
        try:
            website_url = f"https://labs.google/fx/tools/flow/project/{project_id}"
            debug_logger.log_info(
                f"[BrowserCaptcha] 为 project_id={project_id} 创建常驻标签页，访问: {website_url}"
            )
            ...
            for retry in range(60):
                try:
                    ...
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] 等待页面异常: {e}，重试 {retry + 1}/60...")
                    await asyncio.sleep(1)

            if not page_loaded:
                debug_logger.log_error(
                    f"[BrowserCaptcha] 页面加载超时 (project: {project_id})")
                try:
                    await tab.close()
                except:
                    pass
            ...
            if not recaptcha_ready:
                debug_logger.log_error(
                    f"[BrowserCaptcha] reCAPTCHA 加载失败 (project: {project_id})")
                try:
                    await tab.close()
                except:
                    pass
            ...
            debug_logger.log_info(
                f"[BrowserCaptcha] ✅ 常驻标签页创建成功 (project: {project_id})")
    ...
    async def _close_resident_tab(self, project_id: str):
        ...
        if resident_info and resident_info.tab:
            try:
                await resident_info.tab.close()
                debug_logger.log_info(
                    f"[BrowserCaptcha] 已关闭 project_id={project_id} 的常驻标签页")
            except Exception as e:
    ...
    async def _get_token_legacy(self, project_id: str) -> Optional[str]:
        ...
        try:
            website_url = f"https://labs.google/fx/tools/flow/project/{project_id}"
            debug_logger.log_info(
                f"[BrowserCaptcha] [Legacy] 访问页面: {website_url}")
            ...
            # 执行 reCAPTCHA
            debug_logger.log_info(
                "[BrowserCaptcha] [Legacy] 执行 reCAPTCHA 验证...")
        ...
        try:
            if self.browser:
                try:
                    ...
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] 关闭浏览器时出现异常: {str(e)}")
                finally:
                    self.browser = None
    ...
    async def refresh_session_token(self, project_id: str) -> Optional[str]:
        ...
        async with self._resident_lock:
            ...
            if resident_info is None:
                ...
                if resident_info is None:
                    debug_logger.log_warning(
                        f"[BrowserCaptcha] 无法为 project_id={project_id} 创建常驻标签页"
                    )
                    return None
                self._resident_tabs[project_id] = resident_info
